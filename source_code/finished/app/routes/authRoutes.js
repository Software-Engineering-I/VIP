    ...
    apiRouter.route('/google')
        .post(function(req, res) {
            ...
            request.get({ url: peopleApiUrl, headers: headers, json: true }, function(err, response, profile) {
                ...
                User.findOne({'email' : profile.email}, function(err, userreg) {
                    //Added this line to not crash server if the user isn't in our database.
                    if(userreg == null){
                        res.json({message: 'User is not in our database, please register.'});
                        return;
                    }
